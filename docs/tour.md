...
One window holds the whole flow. A searchable **component palette** sits on the
left, the **canvas** in the middle, and docked **Properties**, **Layers**,
**DRC**, and **Console** panels on the right and bottom. Toolbars across the top
carry the place / route / measure tools and the grid, snap, units, export, and
simulate controls.
...
Every gdsfactory PDK (process design kit) component is in the palette, grouped
by kind. Type in the filter box to narrow hundreds of factories down to what you
want, and hover any entry for a live rendered preview before you place it.
...
delete, and zoom. They're all on the **Edit** menu too, with keyboard shortcuts.
...
**cross-section** the route is drawn with, and route with all-angle diagonal
bends or Manhattan. Here's the good part: set a **goal length** and let Phidler
insert an adiabatic meander to hit it, so matched delays come for free.
...
index from your last mode solve. It's the view that matters when you're budgeting
delays.
...
Phidler ships a real [FDTD engine](guide.md#fdtd-simulation) (finite-difference
time-domain, the workhorse method for simulating how light moves through a
structure). Start with the **vertical mode profile**: solve the guided mode of
your waveguide cross-section and see its field and effective index.
...
interferometers. Lay it out visually and export it as GDS.

![A clean add–drop ring resonator laid out on the canvas](screenshots/showcase_ring.png)

Active devices too. Here's a thermo-optic phase shifter, its metal heater and via
