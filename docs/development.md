...
src/phidler/
  ...
  panels/
    ...
    console_panel.py          # interactive Python REPL (code.InteractiveInterpreter) against the live session; optional "Ask Claude" mode
  ...
  ai/                         # optional AI assistant (only imported when the `ai` extra + `claude` CLI are present)
    mcp_server.py             # in-process MCP server exposing the live session; GuiInvoker marshals tool calls onto the Qt thread
    claude_session.py         # drives the `claude` CLI headless (stream-json) as the chat backend behind "Ask Claude"
...
`QT_QPA_PLATFORM=offscreen` and currently covers just under 500 tests.
